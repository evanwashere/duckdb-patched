...
DUCKDB_API void close(duckdb_database *db);
DUCKDB_API void free_blob(duckdb_blob *blob);
DUCKDB_API void disconnect(duckdb_connection *con);
DUCKDB_API void free_result(duckdb_result *result);
DUCKDB_API void free_prepare(duckdb_prepared_statement *prepare);

DUCKDB_API duckdb_connection* connect(duckdb_database* db);
DUCKDB_API duckdb_database* open(bool in_memory, const char* path);
DUCKDB_API duckdb_result* query(duckdb_connection* con, const char* query);
DUCKDB_API duckdb_result* query_prepared(duckdb_prepared_statement* prepare);
DUCKDB_API duckdb_prepared_statement* prepare(duckdb_connection* con, const char* query);

DUCKDB_API uint32_t blob_size(duckdb_blob* blob);
DUCKDB_API uint8_t* blob_data(duckdb_blob* blob);
DUCKDB_API uint32_t row_count(duckdb_result* result);
DUCKDB_API bool row_count_large(duckdb_result* result);
DUCKDB_API uint32_t column_count(duckdb_result* result);
DUCKDB_API uint64_t row_count_slow(duckdb_result* result);
DUCKDB_API const char* result_error(duckdb_result* result);
DUCKDB_API // uint32_t rows_changed(duckdb_result *result);
DUCKDB_API uint32_t param_count(duckdb_prepared_statement* prepare);
DUCKDB_API uint32_t column_type(duckdb_result* result, uint32_t offset);
DUCKDB_API const char* prepare_error(duckdb_prepared_statement* prepare);
DUCKDB_API const char* column_name(duckdb_result* result, uint32_t offset);
DUCKDB_API uint32_t param_type(duckdb_prepared_statement* prepare, uint32_t offset);

DUCKDB_API bool bind_null(duckdb_prepared_statement* prepare, uint32_t offset);
DUCKDB_API bool bind_i8(duckdb_prepared_statement* prepare, uint32_t offset, int8_t value);
DUCKDB_API bool bind_f32(duckdb_prepared_statement* prepare, uint32_t offset, float value);
DUCKDB_API bool bind_u8(duckdb_prepared_statement* prepare, uint32_t offset, uint8_t value);
DUCKDB_API bool bind_i16(duckdb_prepared_statement* prepare, uint32_t offset, int16_t value);
DUCKDB_API bool bind_i32(duckdb_prepared_statement* prepare, uint32_t offset, int32_t value);
DUCKDB_API bool bind_i64(duckdb_prepared_statement* prepare, uint32_t offset, int64_t value);
DUCKDB_API bool bind_f64(duckdb_prepared_statement* prepare, uint32_t offset, double value);
DUCKDB_API bool bind_u16(duckdb_prepared_statement* prepare, uint32_t offset, uint16_t value);
DUCKDB_API bool bind_u32(duckdb_prepared_statement* prepare, uint32_t offset, uint32_t value);
DUCKDB_API bool bind_u64(duckdb_prepared_statement* prepare, uint32_t offset, uint64_t value);
DUCKDB_API bool bind_boolean(duckdb_prepared_statement* prepare, uint32_t offset, bool value);
DUCKDB_API bool bind_timestamp(duckdb_prepared_statement* prepare, uint32_t offset, uint64_t value);
DUCKDB_API bool bind_blob(duckdb_prepared_statement* prepare, uint32_t offset, const void* value, uint32_t length);
DUCKDB_API bool bind_string(duckdb_prepared_statement* prepare, uint32_t offset, const char* value, uint32_t length);
DUCKDB_API bool bind_interval(duckdb_prepared_statement* prepare, uint32_t offset, uint32_t ms, uint32_t days, uint32_t months);

DUCKDB_API int8_t value_i8(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API float value_f32(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint8_t value_u8(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API double value_f64(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API int16_t value_i16(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API int32_t value_i32(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API int64_t value_i64(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint16_t value_u16(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint32_t value_u32(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint64_t value_u64(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API bool value_boolean(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API bool value_is_null(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API char* value_string(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint32_t value_date(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint32_t value_time(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API duckdb_blob* value_blob(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint64_t value_timestamp_ms(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint32_t value_interval_days(duckdb_result* result, uint32_t row, uint32_t column);
DUCKDB_API uint32_t value_interval_months(duckdb_result* result, uint32_t row, uint32_t column);

DUCKDB_API int8_t value_i8_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API float value_f32_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint8_t value_u8_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API double value_f64_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API int16_t value_i16_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API int32_t value_i32_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API int64_t value_i64_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint16_t value_u16_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint32_t value_u32_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint64_t value_u64_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API bool value_boolean_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API bool value_is_null_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API char* value_string_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint32_t value_date_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint32_t value_time_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API duckdb_blob* value_blob_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint64_t value_timestamp_ms_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint32_t value_interval_days_slow(duckdb_result* result, uint64_t row, uint32_t column);
DUCKDB_API uint32_t value_interval_months_slow(duckdb_result* result, uint64_t row, uint32_t column);
