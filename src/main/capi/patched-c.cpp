...
void duckffi_close(duckdb_database *db) { duckdb_close(db); }
void duckffi_disconnect(duckdb_connection *con) { duckdb_disconnect(con); }
...
void duckffi_free_prepare(duckdb_prepared_statement *prepare) { duckdb_destroy_prepare(prepare); }
